...
def update_leaderboard(
    ...
    metric_keys = metric_keys[:5]

    metric_header = " | ".join(metric_keys) if metric_keys else "Metrics"
    lines = [
        "# OCR Leaderboard",
        "",
        ...
        "",
        f"| Rank | Model | Score | {metric_header} | Diff vs Official | Repo |",
        f"| --- | --- | ---: | {' | '.join(['---:' for _ in metric_keys]) if metric_keys else '---'} | ---: | --- |",
    ]
    for item in ranked_entries:
        ...
        metric_text = " | ".join(metric_values) if metric_values else "-"
        diff = item.get("score_diff_vs_official")
        diff_text = "-" if diff is None else f"{float(diff):.4f}"
        lines.append(
            "| {rank} | {model} | {score:.4f} | {metrics} | {diff} | {repo} |".format(
                rank=item.get("rank"),
